...
text = str(
    ...
    F'\n'
    F'The program will ask you if you want to generate a tags file and a config file when you first execute it.\n'
    F'\n'
    F'You can also run the setup again at any time by running the program with the `{__args.setup_option}` from command line.\n'
    F'\n'
    F'The program looks for configuration files in the following order:\n'
    F'\n'
    F'1. In the user config directory.\n'
    F'    1. On Windows that\'s usually `C:\\users\\<user>\\AppData\\Local\\{author}\\{app_name}`\n'
    F'    2. On Ubuntu that\'s usually `/home/<user>/.config/{app_name}`\n'
    F'1. In the system config directory.\n'
    F'    1. On Windows that\'s usually `C:\\ProgramData\\{author}\\{app_name}`\n'
    F'    2. On Ubuntu that\'s usually `/etc/xdg/xdg-ubuntu/{app_name}`\n'
    F'\n'
    ...
    F'    }}\n'
    F'\n'
    F'Tags have the structure `"tag" : "display-name"`. `tag` will be used for the filename whereas `display-name will be displayed in the UI.`\n'
    F'\n'
    ...
    F'\n'
    F'- `{NamingConfigImpl.BASENAME_TAGS_SEPARATOR_KEY}` : the text that will be used to separate the basename from the list of tags. Make sure that this is unique and doesn\'t appear in any tag or in the basename.\n'
    F'- `{NamingConfigImpl.TAG_SEPARATOR_KEY}` : the text that will be used to separate the tags. Make sure this is unique and doesn\'t appear in any tag.\n'
    ...
    F'- `{NamingConfigImpl.REPLACE_BASENAME_SPACER_KEY}` : if this is set to `true` all whitespaces, `{NamingConfigImpl.TAG_SEPARATOR_KEY}` and `{NamingConfigImpl.BASENAME_TAGS_SEPARATOR_KEY}`  in the basename will be replaced by `{NamingConfigImpl.BASENAME_SPACER_KEY}`\n'
    F'- `{NamingConfigImpl.BASENAME_SPACER_KEY}` : the text that will replace any spacers in the basename. See `{NamingConfigImpl.REPLACE_BASENAME_SPACER_KEY}`.\n'
    F'\n'
    F'## Example'
    F'\n'
    ...
    F'\n'
    F'As you can probably guess this is impossible to revert into basename and tags. How is the program '
