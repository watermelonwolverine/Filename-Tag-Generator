...
text = str(
    ...
    F'2. Find out where Python puts your console scripts. It should be somewhere under the `USER_BASE` folder which you can find out using `python.exe -m site`.\n'
    F'3. Either run the program for a first time setup or go directly to [configuration]({configuration.link}) to read about configuration files.\n'
    F'\n'
    F'## Ubuntu\n'
    F'\n'
    ...
    F'    2. Or install directly from the repo with `python -m pip install git+{url}`\n'
